...
class VirtualFileSystem:
    ...
    def _add_file_to_fs(self, name, zip_file):
                ...
                if not part:  # каталог
                    ...
                    try:
                        with zip_file.open(name) as f:
                            cur[part] = f.read().decode("utf-8", errors="ignore")
                    except:
                        cur[part] = ""
            else:
...
class TerminalEmulator:
    def __init__(self, root, vfs_path=None, script_path=None):
        self.root = root
        self.vfs = VirtualFileSystem(vfs_path)  # используем VFS
        self.current_dir = "/"  # внутри виртуальной ФС
        self.script_path = script_path

        # Заголовок
        self.root.title(f"VFS Terminal Emulator - {vfs_path or 'default'}")
        ...
        self.output_text.insert(tk.END, f"[DEBUG] Using VFS: {vfs_path or 'empty'}\n")
        self.output_text.config(state=tk.DISABLED)

        # Приветствие
        self.output_text.config(state=tk.NORMAL)
        self.output_text.insert(tk.END, f"VFS Terminal Emulator v2.0\nCurrent VFS directory: {self.current_dir}\n$ ")
    ...
    def execute_ls(self, args):
        try:
            files = self.vfs.list_dir(self.current_dir)
            for f in files:
    ...
    def execute_cd(self, args):
        ...
        if not self.vfs.is_dir(new_dir):
            self.output_text.insert(tk.END, f"Error: Directory '{new_dir}' does not exist in VFS\n")
            return
